...
class PWAssist {
    ...
    const PWA_CONFIGFILE = 'initializer/pwa-config/pwa_config.xml';
    const VERSION = '0.10';

    static $BR = '<br>';

    private static $_inConcole = false;
    protected static $_fl = array();

    protected static $_skip_keys = array(
      'created_date','fileTypes','ignoreFolders','ignoreFiles','fileSizeLimit','dynamicTest'
    );

    // default configuration
    protected static $defaultCfg = array(
      ...
      ,'pwaConfigFile' => 'initializer/pwa-config/pwa_config.xml'
    ...
    */
    ...
    public static function loadConfig($options='') {
        ...
        $config_file = $_GET['pwaConfigFile'] ?? self::PWA_CONFIGFILE;

        if (is_file(PROJECT_ROOT.$config_file)) {
            try {
                $xml = simplexml_load_file(PROJECT_ROOT.$config_file);
            }
            catch (Exception $e) {
...
EOJS;
        ...
        return file_put_contents(PROJECT_ROOT.self::$cfg['swFilename'], $swCode);
    ...
    public static function saveParameters($options='') {
        ...
        $cfgbody .= "</PWA-config>";
        file_put_contents(PROJECT_ROOT. ($pars['pwaConfigFile'] ?? self::PWA_CONFIGFILE), $cfgbody);
    ...
    */
    public static function createManifest() {
       ...
       foreach(explode(',',$cfg['iconResolutions']) as $res) {
           $rr = "{$res}x{$res}";
           $iconName = str_replace('{size}', "$res" , $cfg['startUri'].$cfg['iconFilenameTemplate']);
...
EOJSON;
        ...
        return file_put_contents(($cfg['startFolder'] . self::$cfg['manifestFilename']), $manifestBody);
    ...
    public static function createIcons() {
        ...
        self::$cfg['baseIcon'] = self::$cfg['startFolder'].self::$cfg['baseIcon'];
        ...
        foreach(explode(',', self::$cfg['iconResolutions']) as $res) {
            ...
            $iconName = str_replace('{size}', "$res" , PROJECT_ROOT.self::$cfg['iconFilenameTemplate']);
    ...
    private static function _scanDir($foldername) {
        ...
        if (count($files)) foreach($files as $onef) {
            ...
            if ($basename === PROJECT_ROOT.self::$cfg['swFilename']) continue;
            if ($basename === PROJECT_ROOT.self::MANIFEST_NAME) continue;
